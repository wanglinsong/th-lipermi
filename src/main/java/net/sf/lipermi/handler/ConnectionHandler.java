...
public class ConnectionHandler implements Runnable {
    ...
    public static ConnectionHandler createConnectionHandler(Socket socket, CallHandler callHandler,
            IProtocolFilter filter) {
        ConnectionHandler connectionHandler = new ConnectionHandler(socket, callHandler, filter);

        String threadName = String.format("ConnectionHandler (%s:%d)", socket.getInetAddress().getHostAddress(), socket
                .getPort()); //$NON-NLS-1$
    ...
    public static ConnectionHandler createConnectionHandler(Socket socket, CallHandler callHandler,
            IProtocolFilter filter, IConnectionHandlerListener listener) {
    ...
    private final List<IConnectionHandlerListener> listeners = new LinkedList<>();
...
//								System.out.println("(" + remoteCall.getCallId() + ") " + remoteReturn.getRet());
    ...
    final synchronized Object remoteInvocation(final Object proxy, final Method method, final Object[] args)
            throws Throwable {
